...
import { N, ZeroAddress } from "ethers";
  ...
  describe("RiceCoin", function () {
    ...
    async function deployFixture() {
      ...
      const RiceSwap = await ethers.getContractFactory("RiceSwap20");
    ...
    describe("Deployment", function () {
      ...
      it("Should payholders", async function () {
        const { riceswap, coin, usdt, otherAccount, owner} = await loadFixture(deployFixture);
          ...
          expect(await usdt.balanceOf(otherAccount.address)).to.equal(95_000_000_000_000_000_00n);
      ...
      it("Should safetransfer", async function () {
        ...
        expect(await usdt.balanceOf(ADMIN.admin)).to.equal(1360450000000000000000000000n)
      });
