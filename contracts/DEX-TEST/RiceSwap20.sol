...
contract RiceSwap20 is IRiceSwapErrors, RCNOwnable, Validators, ReentrancyGuard, RCNBlackList, RCNSecurity{
    ...
    function createPool(
        ...
        emit NewPool(_token0, msg.sender, "fixed income");
    ...
    function payholders(address _token0) external nonReentrant blackList(_token0) pausable {
        ...
        uint64 index = pools[_token0].index;

        uint256 txMonth = ((amount * fee) / index) * totalAmount;
    ...
    function updateFee(uint8 _fee) external onlyAdmin {
        dexFee = _fee;
    }
    ...
    function viewFee() external view returns(uint8){
        return dexFee;
    }
